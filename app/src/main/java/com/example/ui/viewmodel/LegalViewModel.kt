...
class LegalViewModel(private val repository: LegalRepository) : ViewModel() {
    ...
    init {
        ...
        viewModelScope.launch {
            ...
            // Check if there are no cases, and if so populate premium default cases, events, documents, and chat
            if (repository.allCaseFiles.first().isEmpty()) {
                // Populate default case 1: İş Sözleşmesi Feshi ve Kıdem Tazminatı Uyuşmazlığı
                val caseId1 = repository.insertCaseFile(
                    CaseFile(
                        title = "İş Sözleşmesi Feshi ve Kıdem Tazminatı",
                        clientName = "Murat Yılmaz",
                        description = "Müvekkil Murat Yılmaz'ın 5 yıldır çalıştığı bilişim firmasındaki iş akdi, herhangi bir haklı gerekçe gösterilmeksizin ve ihbar süresine uyulmaksızın sözlü olarak feshedilmiştir. Kıdem, ihbar, fazla mesai ve yıllık izin ücretlerinin tahsili talep edilmektedir.",
                        status = "active",
                        notes = "Notlar: Arabuluculuk başvurusu yapıldı, ilk oturum tarihi bekleniyor. Tanıkların listesi hazırlandı."
                    )
                )
                
                // Add default documents for Case 1
                repository.insertDocument(
                    LegalDocument(
                        caseFileId = caseId1.toInt(),
                        name = "İş_Sözleşmesi.pdf",
                        type = "Sözleşme",
                        uploadDate = "10.07.2026",
                        content = "MÜŞTERİ HİZMETLERİ VE YAZILIM HİZMETLERİ SÖZLEŞMESİ\nTaraflar: Murat Yılmaz & TeknoCore A.Ş.\nTarih: 12.04.2021\nMadde 4: Aylık Net Ücret 45.000 TL'dir. Fazla mesailer ücrete dahildir.\nMadde 12: Fesih halinde ihbar öneli 8 haftadır.\nİmza: Murat Yılmaz (İmzalı) / TeknoCore A.Ş. Yetkilisi (İmzasız - Eksik!)",
                        summary = "Murat Yılmaz ile TeknoCore A.Ş. arasındaki 12.04.2021 tarihli iş sözleşmesi. Sözleşmede aylık net ücretin 45.000 TL olduğu, fazla mesailerin ücrete dahil edildiği yazmaktadır. Sözleşmenin işveren tarafındaki yetkili imzasının eksik olduğu tespit edilmiştir.",
                        isUnreadable = false,
                        missingRequiredDocs = "• Fesih Bildirimi\n• İhbarname\n• Ücret Bordroları"
                    )
                )
                ...
                repository.insertDocument(
                    LegalDocument(
                        caseFileId = caseId1.toInt(),
                        name = "Hesap_Dökümü_Banka.pdf",
                        type = "Finans",
                        uploadDate = "12.07.2026",
                        content = "T.C. GARANTİ BANKASI\nMüşteri: Murat Yılmaz\nSon 3 aylık maaş transferleri: TeknoCore A.Ş. tarafından her ayın 5'inde 45.000 TL 'Maaş Ödemesi' açıklamasıyla yapılmıştır.",
                        summary = "Garanti Bankası hesap dökümü. Son 3 aya ait 45.000 TL'lik maaş ödemelerinin düzenli olarak yapıldığını kanıtlamaktadır.",
                        isUnreadable = false,
                        missingRequiredDocs = ""
                    )
                )

                // Add default events for Case 1
                repository.insertEvent(
                    CalendarEvent(
                        caseFileId = caseId1.toInt(),
                        title = "Zorunlu Arabuluculuk İlk Oturumu",
                        type = "Arabuluculuk",
                        date = "2026-07-20",
                        description = "Çağlayan Arabuluculuk Bürosu, saat 14:00. Murat Yılmaz uyuşmazlığı.",
                        isCompleted = false
                    )
                )
                repository.insertEvent(
                    CalendarEvent(
                        caseFileId = caseId1.toInt(),
                        title = "Dava Açma Hak Düşürücü Süre Sonu",
                        type = "Frist",
                        date = "2026-08-15",
                        description = "Arabuluculuk son tutanağının anlaşamama ile bitmesi halinde 2 haftalık dava açma süresi takip edilmelidir.",
                        isCompleted = false
                    )
                )

                // Add some default chat messages for Case 1
                repository.insertChatMessage(
                    ChatMessage(
                        caseFileId = caseId1.toInt(),
                        sender = "USER",
                        message = "Merhaba, bu dosyada sözleşmedeki fazla mesai maddesi geçerli midir?"
                    )
                )
                repository.insertChatMessage(
                    ChatMessage(
                        caseFileId = caseId1.toInt(),
                        sender = "AI",
                        message = "Merhaba! İş sözleşmesindeki 'fazla mesailer aylık ücrete dahildir' hükmü, Yargıtay kararlarına göre yıllık en fazla 270 saate kadar geçerlidir. Eğer müvekkilinizin haftalık çalışması 45 saati aşıyorsa ve yıllık toplamda 270 saatten fazla çalışmışsa, aşan kısımlar için fazla mesai ücreti talep edebiliriz. Ayrıca banka dökümlerinde fazla mesai tahakkuku görünmemektedir."
                    )
                )

                // Populate default case 2: Kira Uyuşmazlığı ve Tahliye Davası
                val caseId2 = repository.insertCaseFile(
                    CaseFile(
                        title = "Kira Uyuşmazlığı ve Tahliye Davası",
                        clientName = "Ayşe Demir",
                        description = "Müvekkil Ayşe Demir, mülkiyeti kendisine ait olan Beşiktaş'taki dairenin kiracısı Ahmet Kaya'nın kira bedelini son 3 aydır ödememesi sebebiyle tahliye davası açmak istemektedir. İki haklı ihtarname gönderilmiştir.",
                        status = "active",
                        notes = "Notlar: İhtarnameler noter onaylıdır ve kiracıya tebliğ edilmiştir. İlamsız takip başlatılacak."
                    )
                )

                repository.insertEvent(
                    CalendarEvent(
                        caseFileId = caseId2.toInt(),
                        title = "İcra Takibine İtiraz Süresinin Sonu",
                        type = "Frist",
                        date = "2026-07-25",
                        description = "Ödeme emrinin tebliğinden itibaren 7 günlük itiraz süresi takip edilecektir.",
                        isCompleted = false
                    )
                )
            }
    ...
    fun selectCaseFile(id: Int?) {
        ...
        _caseAnalysisState.value = UiState.Idle
    }
